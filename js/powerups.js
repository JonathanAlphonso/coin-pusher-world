...
const PowerUps = {
  ...
  // Temporary abilities (can be triggered)
  abilities: {
    coinShower: {
      id: "coinShower",
      name: "Coin Shower",
  ...
  upgrade: function (powerUpId) {
    ...
    if (this.levels[powerUpId] < type.maxLevel) {
      ...
      this.applyEffects();
      UI.updatePowerUp(powerUpId, this.levels[powerUpId]);
      return true;
    }
    return false;
  },
  ...
  applyEffects: function () {
    // Queue speed
    Coins.setQueueSpeed(this.getEffect("queueSpeed"));

    // Queue capacity
    Coins.setMaxQueue(Math.floor(this.getEffect("queueCapacity")));

    // Coin value
    Coins.setValueMultiplier(this.getEffect("coinValue"));

    // Multi drop is handled in Game.dropCoin()
    // Lucky coins is handled in Coins.dropCoin()
  },
  ...
  doMagneticPull: function () {
    UI.showMessage("🧲 MAGNETIC PULL!");
    Sound.play("ability");

    // Push all coins forward
    ...
    // Also push collectibles
    if (Collectibles && Collectibles.activeItems) {
      for (const item of Collectibles.activeItems) {
        if (item.body) {
          item.body.vz = 3;
          item.body.vy = 0.5;
        }
  ...
  showSelection: function () {
    ...
    // Guard against multiple calls - if menu is already visible, don't pause again
    ...
    if (offered.length === 0) {
      // All maxed out, give bonus coins instead
      Coins.addToQueue(10);
      UI.showMessage("All maxed! +10 Queue!");
      return;
    }
    ...
    for (let i = 0; i < offered.length; i++) {
      ...
      optionDiv.innerHTML = `
                <div class="upgrade-name">${type.icon} ${type.name} (Lv ${
        currentLevel + 1
      })</div>
  ...
  selectUpgrade: function (powerUpId) {
    ...
    const type = this.types[powerUpId];
    UI.showMessage(type.icon + " " + type.name + " upgraded!");
  },
  ...
  loadSaveData: function (data) {
    if (data && data.levels) {
      ...
      for (const key in this.levels) {
        UI.updatePowerUp(key, this.levels[key]);
      }
    }
  },
};
