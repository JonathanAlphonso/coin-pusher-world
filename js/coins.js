...
const Coins = {
  ...
  spawnCoin: function (x, y, z, type) {
    ...
    coin.body = Physics.createBody({
      ...
      restitution: 0.25,
      mesh: coin.mesh,
      // Initial rotation - coins start flat (lying down)
      rx: Math.PI / 2,
      ry: 0,
      rz: 0,
      data: { coin: coin },
    ...
    // Add small random velocity for natural spread
    coin.body.vx = Utils.random(-1, 1);
    coin.body.vy = Utils.random(-0.5, 0);

    // Small initial spin
    coin.body.ay = Utils.random(-2, 2);
  ...
  onCoinFallOff: function (coin, body) {
    ...
    const comboMult = typeof Combo !== "undefined" ? Combo.getMultiplier() : 1;

    // Contribute to jackpot
    if (typeof Jackpot !== "undefined") {
      Jackpot.contribute(coin.value);
    }

    // Track for bonus wheel
    if (typeof BonusWheel !== "undefined") {
      BonusWheel.onCoinDropped();
    }
    ...
    if (bonusResult) {
      if (bonusResult.type === "miss") {
        ...
        Game.addScore(Math.floor(coin.value * 0.1), body.x, body.y, body.z, 1);
        ...
        Game.addScore(coin.value, body.x, body.y, body.z, totalMult);
        UI.showCoinScore(Math.floor(coin.value * totalMult), { x: body.x, y: body.y, z: body.z });

        if (bonusResult.isBonus) {
          // Bonus slots trigger wheel or powerup
          if (typeof BonusWheel !== "undefined" && Math.random() < 0.5) {
            BonusWheel.trigger("bonusSlot");
          } else {
            PowerUps.showSelection();
          }
        }

        // Chance to spawn collectible
        if (typeof Collectibles !== "undefined") {
          Collectibles.maybeSpawn(body.x, body.y + 2, body.z);
        }
      } else {
        // Regular bonus zone
        this.triggerBonus(bonusResult, body.x, body.y);
        Game.addScore(coin.value, body.x, body.y, body.z, comboMult);
        UI.showCoinScore(Math.floor(coin.value * comboMult), { x: body.x, y: body.y, z: body.z });
      }
    } else {
  ...
  checkBonusZones: function (x, y, z) {
    for (let i = 0; i < Board.bonusZones.length; i++) {
      const zone = Board.bonusZones[i];
    ...
    // This ensures coins always get credit even if they miss exact slot bounds
    if (typeof Board !== 'undefined' && y < Board.scoringY + 1) {
  ...
  triggerBonus: function (zone, x, y) {
    switch (zone.type) {
      case "queue":
        const addAmount = Utils.randomInt(3, 8);
        this.addToQueue(addAmount);
        UI.showMessage("+" + addAmount + " Coins!", x, y);
        Sound.play("bonus");
        break;
  ...
  dropCoin: function () {
    ...
    this.coinQueue--;
    UI.updateQueue(this.coinQueue);

    // Random X position within drop zone
    const dropX = Utils.random(dropZone.minX, dropZone.maxX);
    const dropY = dropZone.y;
    const dropZ = dropZone.z;

    // Determine coin type with luck factor
    const luckyChance = PowerUps.getLuckyChance();
    let type = "gold";
